...
def main(ip=getServerIP(), isAdmin=False):
    ...
    if userInput == 1:
        ...
        a = requests.get(ip+'/newStudent/%s/%s/%s/%s/%s' % (userinput_NAME,
                         userinput_chinese, userinput_math, userinput_english, userinput_ID))
        ...
        if not a.text == "False":
            print("ID:%s   名字：%s   语文分数:%s   数学分数：%s   英语分数：%s" % (eval(a.text)['id'], eval(a.text)[
                  'studentName'], eval(a.text)['chineseScore'], eval(a.text)['mathScore'], eval(a.text)['englistScore']))
        main(isAdmin=isAdmin, ip=ip)
    elif userInput == 3:
        ...
        for i in eval(a.text):
            print("ID:%s   名字：%s   语文分数:%s   数学分数：%s   英语分数：%s" % (
                i['id'], i['studentName'], i['chineseScore'], i['mathScore'], i['englistScore']))
        main(isAdmin=isAdmin, ip=ip)
    elif userInput == 5:
        ...
        if isAdmin:
            if str(userInput)[0] == '1':
                ...
                a = requests.get(ip+'/newUser/%s/%s/%s' %
                                 (userName, userPwd, userIsAdmin))
