...
def getServerConfig(dataName="users.sso-server-config"):
    ...
    for i in configFileText_:#循环 去掉\n
        configFileText.append(eval(i.strip()))#经测试确认此处使用for i in configFileText和strip和eval无效 建立新的列表
...
def newUser(userName,userPwd,isAdmin):
    with open("users.sso-server-config","a",encoding="utf-8")as configFile:#以追加模式打开用户信息文件
        userinfo={'userName':userName,"userPwdMD5":md5(userPwd),'admin':bool(isAdmin)}#生成信息
        log('管理员正在新建名称为%s,密码为%s，管理员权限为%s的账户'%(userName,userPwd,str(isAdmin)))
...
def newStudent(name,chineseScore,mathScore,englishScore,studentID):
    with open("datas.sso-server-config","a",encoding="utf-8")as dataFile:#打开文件，把URL后面的5个值对应到词典中
        studentInfo={'studentName':name,"chineseScore":chineseScore,"mathScore":mathScore,"englistScore":englishScore,"id":studentID}
...
def delUser(userName):
    ...
    configFileText=getServerConfig(dataName='users.sso-server-config')#for循环+判断 remove移除
